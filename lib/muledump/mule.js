        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            var dobp = this.opt('backpack') && +c.HasBackpack;
                ...
                if ( this.totals.cache === false ) {
                    ...
                    for ( var te = 0; te < 4; te++ ) this.totals.count(eq[te], 'equipment');
                    for ( var ti = 4; ti < 8; ti++ ) this.totals.count(eq[ti], 'inv');
                    for ( var tb = 8; tb < eq.length; tb++ ) this.totals.count(eq[tb], 'backpack');
        ...
        if ( d.Account.Vault && this.totals.cache === false ) {
            ...
            for ( var tv = 0; tv < chests.length; tv++ )
                if ( chest = chests[tv].split(',') )
                    for ( var tv2 = 0; tv2 < chest.length; tv2++ )
                        this.totals.count(chest[tv2], 'vaults');
