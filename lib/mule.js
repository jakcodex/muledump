...
Mule.prototype.queueStart = function(guid=false, self=false, action=false) {
    ...
    if ( window.MuleQueue.done == 0 && window.MuleQueue.busy == false ) window.techlog("MQ/TaskBegin", 'force');

	//  run this item if there is no active task
	if ( window.MuleQueue.busy == false ) {
...
Mule.prototype.queueFinish = function(guid, status) {

    if ( window.MuleQueue.mules[guid] ) {

        date = new Date();
        window.MuleQueue.mules[guid].endTime = date.toISOString();
        ...
        window.techlog("MQ/QueueFinish '" + status + "' " + guid + " in " + (window.MuleQueue.mules[guid].runtime/1000) + " seconds", 'force');
        window.techlog(JSON.stringify(window.MuleQueue.mules[guid]));
        if ( window.MuleQueue.done == window.MuleQueue.total ) {

            window.MuleQueue.endTime = new Date().toISOString();
            ...
            window.techlog("MQ/TaskFinished in " + (window.MuleQueue.runtime/1000) + " seconds", 'force');

        }

	} else window.techlog("MQ/QueueError - Unknown queue ID: " + guid, 'force');
...
Mule.prototype.query = function(ignore_cache) {
	...
	if (!ignore_cache) {
		...
		if (c) {
			this.parse(c);
			this.busy = false;
			this.queueFinish(this.guid, 'cache');
			return;
		}
	}
	...
	function onResponse(xml) {
        ...
        if ( error = xml.match(/^<Error\/?>(.*?)<\/?Error>$/) ) {
            self.queueFinish(self.guid, 'error');
            ...
            var res = data.query.results;
            window.techlog("Response data for " + self.guid);
            window.techlog(JSON.stringify(data));
            ...
            if (!res.Chars) {
                //( res.Error ) ? self.error('char error: ' + JSON.stringify(res.Error)) : self.error('bad reply: ' + JSON.stringify(data));
                return;
            }
...
/*	if (this.data && compare(d, this.data.query.results.Chars)) {
	...
	if (this.opt('vaults')) {
		...
		window.vaultlayout = ( this.opt('vaultlayout') ) ? this.opt('vaultlayout') : window.vaultlayout;
		VAULTORDER = vaultorders[window.vaultlayout].vaultorder;
        ...
        vaultlayout = ( window.vaultlayout && typeof vaultorders[window.vaultlayout] === 'object' ) ? window.vaultlayout : 1;
		vaultwidth = ( typeof vaultorders[vaultlayout] === 'object' && vaultorders[vaultlayout].vaultwidth ) ? vaultorders[vaultlayout].vaultwidth : ROW;
		...
		chests = w[1];
        window.techlog("-- Begin Chests JSON --");
