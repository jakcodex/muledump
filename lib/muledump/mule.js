(function ($, window) {
    ...
    function arrangevaults(v, layout) {
        ...
        //quickfix for setting vaultwidth without dealing with vaultbuilder
        //var vaultwidth = setuptools.data.muledump.vaultbuilder.layouts[layout].vaultwidth;
        var vaultwidth = setuptools.data.config.rowlength;
    ...
    Mule.prototype.query = function (ignore_cache, cache_only, freshness, MuleQueue) {
        ...
        $.ajax({
            ...
            success: function (xml, textStatus, request) {
                ...
                } else if (xml === "<Error>Internal error, please wait 5 minutes to try again!</Error>") {
                    self.queueFinish(self.guid, 'hardRateLimited', 'Request was rate limited by Appspot');
                    console.log('Your IP is being rate limited by Deca. Waiting 5 minutes to retry. test');
    ...
    Mule.prototype.parse = function (data, skipCacheWrite) {
        ...
        */
        ...
        var materials = d.Account.MaterialStorage.Chest.join(',');
        ...
        if (this.opt('materials')) {
            ...
            if (Array.isArray(items) === true) {
                ...
                if (this.loginOnly === false && !this.opt('shrink')) maketable('materials', marr, ((setuptools.data.config.giftChestWidth === 0) ? setuptools.data.config.rowlength : setuptools.data.config.giftChestWidth)).appendTo(this.dom);
            }
        ...
        if (this.opt('spoils')) {
            ...
            if (Array.isArray(items) === true) {
                ...
                if (this.loginOnly === false && !this.opt('shrink')) maketable('spoils', sarr, ((setuptools.data.config.giftChestWidth === 0) ? setuptools.data.config.rowlength : setuptools.data.config.giftChestWidth)).appendTo(this.dom);
            }
        ...
        if (this.opt('ssnlpetinv')) {
            ...
            if (Array.isArray(items) === true) {
                ...
                if (this.loginOnly === false && !this.opt('shrink')) maketable('ssnlpetinv', sarr, ((setuptools.data.config.giftChestWidth === 0) ? setuptools.data.config.rowlength : setuptools.data.config.giftChestWidth)).appendTo(this.dom);
            }
        ...
        var petinv;
        for (let index = 0; index < d.Char.length; index++) 
            if(d.Char[index].Seasonal === "False") 
        ...
        if (this.opt('petinv')) {
            ...
            if (Array.isArray(items) === true) {
                ...
                if (this.loginOnly === false && !this.opt('shrink')) maketable('petinv', sarr, ((setuptools.data.config.giftChestWidth === 0) ? setuptools.data.config.rowlength : setuptools.data.config.giftChestWidth)).appendTo(this.dom);
            }
