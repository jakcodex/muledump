...
Mule.prototype.queueStart = function(guid=false, self=false, action=false) {
    ...
    if ( !window.MuleQueue.mules[guid] ) {
    	...
    	if ( window.debugging == true ) console.log("MQ/QueueAdd " + guid);
    ...
    if ( window.MuleQueue.done == 0 && window.MuleQueue.busy == false ) console.log("MQ/TaskBegin");
	...
	if ( window.MuleQueue.busy == false ) {
        ...
        if ( window.debugging == true ) console.log("MQ/QueueRun " + guid);
...
Mule.prototype.queueFinish = function(guid, status) {
    ...
    if ( window.MuleQueue.mules[guid] ) {
        ...
        console.log("MQ/QueueFinish '" + status + "' " + guid + " in " + (window.MuleQueue.mules[guid].runtime/1000) + " seconds");
        if ( window.debugging == true ) console.log(window.MuleQueue.mules[guid]);
        if ( window.MuleQueue.done == window.MuleQueue.total ) {
        	...
        	console.log("MQ/TaskFinished in " + (window.MuleQueue.runtime/1000) + " seconds");

        }

	} else console.log("MQ/QueueError - Unknown queue ID: " + guid);
...
Mule.prototype.query = function(ignore_cache) {
	...
	function onResponse(data) {
        ...
        date = new Date();
        data = {query: {created: date.toISOString(), updated: date.toISOString(), results: xmlToJson(parser.parseFromString(data,"text/xml"))}};
        /*
		...
		*/
        ...
        //  replacing yql with a direct XMLDOM object
        if ( data.html ) {
            self.error("server error");
            if ( data.html.body.parsererror ) {
                self.log('full error:' + JSON.stringify(data.html.body.parsererror))
            }
            return;
        }

        var res = data.query.results;
        if ( window.debugging == true ) console.log("Response data for " + self.guid);
        if ( window.debugging == true ) console.log(data);
		...
		if (!res.Chars) {
			self.error(res.Error || 'bad reply: ' + JSON.stringify(data))
			return;
		}
...
/*	if (this.data && compare(d, this.data.query.results.Chars)) {
	...
	if (this.opt('vaults')) {
		...
		for (i = 0; i < chests.length; i++) {
			if (chests[i] === 0 || Object.keys(chests[i]).length === 0) {
				arr.push(null);
				continue;
			}
