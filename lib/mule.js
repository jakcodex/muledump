...
Mule.prototype.queueStart = function(guid=false, self=false, action=false) {
    ...
    if ( window.MuleQueue.done == 0 && window.MuleQueue.busy == false ) {
    	window.techlog("MQ/TaskBegin", 'force');
    	setTimeout(function() {
	...
	if ( (window.RateLimitExpiration <= Date.now()) && window.MuleQueue.busy == false ) {
...
Mule.prototype.queueFinish = function(guid, status) {
    ...
    if ( window.MuleQueue.mules[guid] ) {
        ...
        window.techlog("MQ/QueueFinish '" + status + "' " + guid + " in " + (window.MuleQueue.mules[guid].runtime/1000) + " seconds", ( status === 'cache' ) ? 'string' : 'force');
