...
function update_totals() {
	...
	function isbad(id) {
		if (!options.famefilter && !options.feedfilter) return false;
		var i = items[id] || items[-1];
        if (options.famefilter && i[5] > +options.fameamount) return false;
        if (options.feedfilter && i[6] > +options.feedpower) return false;
		return true;
	}
