    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        */
        ...
        if (this.opt('potions')) {
            ...
            if ( this.loginOnly === false && !this.opt('shrink') ) this.dom.append($('<hr class="potions">'));
            if ( Array.isArray(items) === true ) {

                var parr = [];
                while (items.length) {
                    while (items.length < 8) items.push("-1");
                    parr.push(makechest(items, 'potions'));
                    items = items.slice(8);
                }
                if ( this.loginOnly === false && !this.opt('shrink') ) maketable('potions', parr, ( (setuptools.data.config.giftChestWidth === 0) ? setuptools.data.config.rowlength : setuptools.data.config.giftChestWidth )).appendTo(this.dom);
            }
