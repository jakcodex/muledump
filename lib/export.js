...
function export_totals() {
	...
	if ( type == 'text' || type == 'csv' || type == 'json' ) {
		text = txtaggr(type);
        text = text.replace(/\n/g, "<br>");
        $.featherlight("<p>" + text + "</p>");
		return;
	}
...
function render_totals(callback) {
	...
	var img = new Image(),
		...
		ct = c.getContext("2d");
	img.src = ( window.RemoteRendersURL ) ? window.RemoteRendersURL : "https://raw.githubusercontent.com/jakcodex/muledump/master/lib/renders.png";

	window.techlog("Export using URL: " + img.src);
