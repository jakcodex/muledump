...
setuptools.app.groups.load = function(accounts) {
    ...
    for ( i = 0; i < setuptools.data.groups.groupOrder.length; i++ ) {
        ...
        for ( x = 0; x < groupData.members.length; x++ ) {
            ...
            if ( typeof setuptools.data.accounts.accounts[groupData.members[x]] === 'undefined' ) {
                ...
                trigger = true;
                setuptools.data.groups.groupList[groupName].members.splice(x, 1);

            }

        }

        //  if triggered we will compact this group
        if ( trigger === true ) {

            saveConfig = true;
            setuptools.data.groups.groupList[groupName].members.filter(function (item) {
    ...
    if ( saveConfig === true ) setuptools.app.config.save('GroupsManager/InitCleanup');
