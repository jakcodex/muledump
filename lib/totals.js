...
function update_totals() {
	...
	for (var i in mules) {
		...
		//  process vaults and gift chests
		var startIndex = 0;
		var lastIndex = m.vaults.length;
		if ( mules[i].opt('vaults') === false ) startIndex = m.vaults.length-1;
		if ( mules[i].opt('gifts') === false ) lastIndex--;
