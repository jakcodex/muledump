...
function addcustomchsort(mule) {
    ...
    var custchsort = $('<div class="button">');
    custchsort.text('+');
...
Mule.prototype.queueWait = function(guid, self, action) {
...
/*	if (this.data && compare(d, this.data.query.results.Chars)) {
	...
	var arr = [];
	//  custom
    if ( this.opt('chsort') === '3' ) {
        if ( setuptools.data.muledump.chsortcustom[this.guid] ) {
            ...
            for ( var i in d.Char ) {
                ...
                if (d.Char.hasOwnProperty(i)) {
                    ...
                    var index = setuptools.data.muledump.chsortcustom[this.guid].indexOf(d.Char[i].id);
