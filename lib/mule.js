...
// dom snippet generators
    ...
    function addmainbutton(mule, target) {
        ...
        button.click(function(){ mule.reload() });
...
//  add task to the MuleQueue and determine queue eligibility
    Mule.prototype.queueStart = function(guid, self, action) {
        ...
        if ( !window.MuleQueue.mules[guid] ) {
            ...
            window.techlog("MuleQueue/QueueAdd " + guid);
        ...
        if ( RateLimitWarningActive === false && window.MuleQueue.done === 0 && window.MuleQueue.busy === false && window.MuleQueue.pause === false) {
            window.techlog("MuleQueue/TaskBegin at " + (new Date().toISOString()), 'force');
            setTimeout(function() {
                $('#reloader').html('Pause Reload');
            }, 0);
        }
        ...
        if ( window.MuleQueue.busy === false && window.MuleQueue.pause === false && (window.RateLimitExpiration <= Date.now()) ) {
            ...
            window.techlog("MuleQueue/QueueRun " + guid);
...
//  complete a MuleQueue task
    Mule.prototype.queueFinish = function(guid, status, cache_only, errorMessage) {
        ...
        if ( window.MuleQueue.mules[guid] && cache_only === false ) {
            ...
            if ( window.MuleQueue.running === true ) window.techlog("MuleQueue/QueueFinish '" + status + "' " + guid + " in " + (window.MuleQueue.mules[guid].runtime / 1000).toFixed(3) + " seconds (" + remaining + " remaining)", MessageType);

            //  when an account is reloaded less info is necessary (or available)
            if ( window.MuleQueue.running === false ) window.techlog("MuleQueue/TaskFinish '" + status + "' " + guid + " in " + (window.MuleQueue.mules[guid].runtime / 1000).toFixed(3) + " seconds", 'force');
            ...
            if (window.MuleQueue.done === window.MuleQueue.total) {
                ...
                window.techlog("MuleQueue/TaskFinished in " + (window.MuleQueue.runtime / 1000).toFixed(3) + " seconds at " + (new Date().toISOString()), 'force');
                window.MuleQueue.running = false;
        ...
        } else if ( cache_only === false ) window.techlog("MuleQueue/QueueError - Unknown queue ID: " + guid, 'force');
